...
import { createError, createFunction, createNumber, isNumber, isString, LispVal } from "../types.ts";

const parse = createFunction((str: LispVal): LispVal => {
  const err = createError(`Integer/parse: Provided value was not a number, it was a ${str.type}`)
...
const create = createFunction((x: LispVal): LispVal => {
  const err = createError(`Integer/create: Provided value was not a number, it was a ${x.type}`)
